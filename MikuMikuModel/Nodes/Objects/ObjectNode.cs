        ...
        AddCustomHandler("Auto Create Constraints", () =>
        {
            ...
            if (!string.IsNullOrEmpty(objectFarcFilePath))
            {
                if (objectFarcFilePath.EndsWith(".farc", System.StringComparison.OrdinalIgnoreCase))
                    baseObjectSet = BinaryFileNode<ObjectSet>.PromptFarcArchiveViewForm(objectFarcFilePath,
                        "Select the reference object set file.", "This archive has no object set file.");
                else
            ...
            AddRecursively((NodeBlock)Data.Skin.Blocks.FirstOrDefault(x =>
                x is NodeBlock nodeBlock && nodeBlock.ParentName == "n_hara_cp"));
            ...
            void AddRecursively(NodeBlock block)
            {
                ...
                foreach (var alsoBlock in Data.Skin.Blocks.OfType<NodeBlock>()
                             .Where(x => x.ParentName == block.Name ||
                                         (block is OsageBlock osgBlock && x.ParentName == osgBlock.Nodes[0].Name))
                             .OrderBy(x => x.Name))
